...
<body>
<%	
String warnum =new String(request.getParameter("warnum").getBytes("ISO-8859-1"),"utf-8");
String emnum =new String(request.getParameter("emnum").getBytes("ISO-8859-1"),"utf-8");
String watele = request.getParameter("tele").trim();
String remarks =new String(request.getParameter("remarks").getBytes("ISO-8859-1"),"utf-8");

%>

<form action="/courseDesignProject/editwarehouseservlet" method="post">
		<table width="60%" border="1" class="margin" cellspacing="0" style="font-size:18px;font-family:serif;">
			<tr style="background-color:#F8F9FF" >
				<td >备件入库信息登记表（<span class="c_red">*号为必填项</span>）
				</td>
			</tr>
			<tr>
				<td >仓库编号：<%=warnum%><span class="c_red">*</span></td>
			</tr>
			<tr><td>
					仓库负责人工号：<select name="agent" id="agent" class="easyui-combobox" style="width: 100px;">
					<%
			...
			<tr>
				<td >仓库电话：<input id="watele" name="watele" type="text" class="easyui-validatebox" 
			...
			<tr>
				<td colspan="2" ><input name="add" type="submit" value="修改" /></td>
		...
		<input type="hidden" name="warnum" value="<%=warnum %>">
		</form>
<script type="text/javascript">
