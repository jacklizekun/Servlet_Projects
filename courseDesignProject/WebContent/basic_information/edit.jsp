	...
	<table class="easyui-datagrid" toolbar="#toolbar"
		...
		<tbody>
			<%
				...
				String strSql = "select* from spare_parts order by 备件编码";
			...
			<tr>
				...
				<td><div align="center">
					<a href="/courseDesignProject/basic_information/edit2.jsp?name=<%=name%>&code=<%=code%>&unit=<%=unit%>&place=<%=place%>
				&unitprice=<%=unitprice%>&quntity=<%=quntity%>&stan=<%=stan%>&safestoke=<%=safestoke%>
				&pristoke=<%=pristoke%>&pridemand=<%=pridemand%>&ordercost=<%=ordercost%>
				&unitpprice=<%=unitpprice%>">修改</a>
					</div></td>
				<td><div align="center"><a href="/courseDesignProject/deleservlt?code=<%=code%>">删除</a></div></td>
